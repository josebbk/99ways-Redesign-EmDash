...
Applies `DESIGN_SYSTEM.md`. All copy below is **verbatim from the current live site** — do not rewrite, rephrase, or expand it. Where a copy field is genuinely missing (not just unstyled), it's marked `[TODO: ...]` — do not invent replacement text for those either; flag back to a human.
...
- Container max-width `1200px`, centered, responsive padding per `DESIGN_SYSTEM.md` §3.
- Background: `--color-bg` for the entire page — no section-level background changes.
- Sections separated by `--space-24` vertical rhythm (desktop) and a `--color-border` hairline only where a visual break is needed (not between every section — keep it minimal).
...
- Clear space around logo per `DESIGN_SYSTEM.md` §4 — do not let nav items crowd it.

---

## 2. Hero

- No mascot/illustration. Replace with an abstract brand-system visual (not a stock image, not a character illustration) — e.g. a large-scale treatment of the compact mark's pupil geometry, static, on `--color-bg`. **[TODO: final hero visual — flag back for one more round if you want this designed rather than left as a placeholder block.]**
- Headline (`--text-display`, `--color-text`):
  > Find and Pull Your Biggest Growth Levers with Experimentation
- Subhead (`--text-body`, `--color-text-muted`, slightly larger than base body — 1.125rem):
  > We run your CRO operation, from research and prioritization to implementation and analysis.
- CTA (primary button style): **"Book Intro Call"** → `[TODO: link missing — was "#" on the live site, needs a real booking URL]`
...
- Section title (`--text-h2`): exact text above, verbatim.
- **Layout: horizontally-scrolling filmstrip**, not a grid. Cards per `DESIGN_SYSTEM.md` §5 (flat, 1px `--color-border`, no shadow).
...
- Section title (`--text-h2`): exact text above, verbatim.
- **Layout:** 3-column card grid (desktop), 1-column stacked (mobile). Show the latest 3 posts by default (not all 6) to keep the homepage lean — content collection should be sortable by date so this stays current automatically. **[Note: this is a layout recommendation, not a brand rule — say the word if you want all 6 shown instead.]**
...
## 7. Pre-footer CTA (new structural addition)

Not present on the current site — added to give a second conversion point before the footer, on a page this content-light. **Reuses existing CTA copy only, no new text:**

- Text: **"Book Intro Call"** (same button, same styling as hero CTA — `--color-action` primary button)
- Centered, full-width band, generous vertical padding (`--space-24`).
- **[TODO: this section is optional — confirm before build if you'd rather skip it and go straight from Academy grid to footer.]**
...
- **Map embed:** Google Maps, query "99Ways, Mr.Treublaan 7, 1097 DP Amsterdam." Google's embed does not have a native dark variant that matches this palette — either accept the default light embed styled inside a bordered dark card, or replace with a static styled map/illustration. **[Design call, not a copy issue — flag if you want the static-image route instead.]**
